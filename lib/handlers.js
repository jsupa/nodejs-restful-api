...
handlers.sessionDeleted = function (data, callback) {
	if (data.Method === 'get') {
		const templateData = {
			...
			'head.description': 'Yout have been logged out from your account',
...
handlers.accountDeleted = function (data, callback) {
	if (data.Method === 'get') {
		const templateData = {
			...
			'head.description': 'Your account has beed deleted',
...
handlers.checksCreate = function (data, callback) {
	if (data.Method === 'get') {
		const templateData = {
			...
			'body.class': 'cechksCreate',
		};
		helpers.getTemplate('cechksCreate', templateData, (err, str) => {
			if (!err && str) {
				helpers.addUniversalTemplates(str, templateData, (err, str) => {
					if (!err && str) {
						callback(200, str, 'html');
